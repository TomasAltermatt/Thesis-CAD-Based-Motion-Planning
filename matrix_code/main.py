...
if __name__ == "__main__":
    ...
    input_folder = 'matrix_code/STLs/car'
    ...
    # =======================================================
    use_cython = False
    use_parallel_narrow = not use_cython  # Set to True to enable parallel processing for narrow phase interference checks
    
    print(f"\n[STARTING] AABB Pipeline for: {assembly_name}... (use_parallel_narrow={use_parallel_narrow}, use_cython={use_cython})")

    start_time = time.time()
    assembly_manifest_AABB = imf.load_assembly_from_folder(input_folder, bounding_box_type="AABB")

    final_matrices_AABB = imf.calculate_IM_matrices(assembly_manifest_AABB, use_parallel_narrow=use_parallel_narrow, use_cython=use_cython)
