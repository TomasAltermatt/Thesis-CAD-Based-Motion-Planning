...
class SimRenderer:
    @staticmethod
    def replay(sim, record = False, record_path = None, make_video = False, fps = 30):
        if record:
        ...
        if record:
            ...
            if make_video:
                ...
                os.system(f"ffmpeg -framerate {fps} -i {images_path} -c:v libx264 -pix_fmt yuv420p {record_path} -hide_banner -loglevel error")
            else:
