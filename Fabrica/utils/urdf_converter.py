...
class Body:
    ...
    def parse_urdf(self, node):
        ...
        ''' mass from kg to g '''
        if not 'inertial' in self.params:
            print(f'body {self.name} has no inertial, skipped')
            return # NOTE: need to add primitive shape manually
        ...
        if 'visual' not in self.params:
            self.params['visual'] = {}
        if 'origin' not in self.params['visual']:
            self.params['visual']['origin'] = {}
            self.params['visual']['origin']['xyz'] = np.zeros(3)
            self.params['visual']['origin']['rpy'] = np.zeros(3)
        if export_unit == 'cm-g':
        ...
        if 'collision' not in self.params:
            self.params['collision'] = {}
        if 'origin' not in self.params['collision']:
            self.params['collision']['origin'] = {}
            self.params['collision']['origin']['xyz'] = np.zeros(3)
            self.params['collision']['origin']['rpy'] = np.zeros(3)
        if export_unit == 'cm-g':
...
def convert_from_urdf_to_redmax(urdf_path: str, redmax_path: str) -> bool:
    ...
    redmax_visual_mesh_folder = os.path.join(redmax_folder, 'visual')
    redmax_collision_mesh_folder = os.path.join(redmax_folder, 'collision')
    ...
    os.makedirs(redmax_visual_mesh_folder)
    os.makedirs(redmax_collision_mesh_folder)
    ...
    for (_, joint) in joints.items():
        if joint.params['parent']['link'] not in bodies or joint.params['child']['link'] not in bodies:
            continue
    ...
    for (_, body) in bodies.items():
        if 'visual' not in body.params or 'geometry' not in body.params['visual'] or 'mesh' not in body.params['visual']['geometry']:
            continue
        visual_mesh_paths.add(body.params['visual']['geometry']['mesh']['filename'].replace('package://', ''))
    for visual_mesh_pash in visual_mesh_paths:
        ...
        if export_unit == 'cm-g':
            if isinstance(visual_mesh, trimesh.Scene):
                for _, mesh in visual_mesh.geometry.items():
        ...
        with open(os.path.join(redmax_visual_mesh_folder, mesh_filename), 'w', encoding = 'utf-8') as f:
    ...
    for (_, body) in bodies.items():
        if 'collision' not in body.params or 'geometry' not in body.params['collision'] or 'mesh' not in body.params['collision']['geometry']:
            continue
        collision_mesh_paths.add(body.params['collision']['geometry']['mesh']['filename'].replace('package://', ''))
    for collision_mesh_path in collision_mesh_paths:
        ...
        with open(os.path.join(redmax_collision_mesh_folder, mesh_filename), 'w', encoding = 'utf-8') as f:
            collision_mesh.export(f, file_type = 'obj')
        if export_unit == 'cm-g':
    ...
    def export(body, fp, indentation):
        ...
        if body.joint_name is None:
            ...
            joint_type = joints[body.joint_name].params['type']
        ...
        # visual mesh
        if 'visual' in body.params and 'geometry' in body.params['visual'] and 'mesh' in body.params['visual']['geometry']:
        ...
        # contacts
        if 'collision' in body.params and 'geometry' in body.params['collision'] and 'mesh' in body.params['collision']['geometry']:
