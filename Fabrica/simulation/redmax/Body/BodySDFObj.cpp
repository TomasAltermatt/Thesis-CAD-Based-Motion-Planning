...
void BodySDFObj::load_visual_mesh(std::string visual_filename, Vector3 scale) {
    ...
    std::string err;
    tinyobj::LoadObj(&attrib, &obj_shape, &obj_material, &err, visual_filename.c_str(), directory_of(visual_filename).c_str());
    ...
    for (int i = 0;i < num_shapes;i++) {
        num_elements += (int)obj_shape[i].mesh.indices.size() / 3;
    }
    _Fr.resize(3, num_elements);
    int index = 0;
    for (int i = 0;i < num_shapes;i++) {
        for (int j = 0;j < (int)obj_shape[i].mesh.indices.size() / 3;j++) {
            _Fr.col(index) = Vector3i(obj_shape[i].mesh.indices[j * 3].vertex_index,
...
void BodySDFObj::get_rendering_objects(
    ...
    object_option.SetVectorOption("ambient", _color(0) * 0.8, _color(1) * 0.8, _color(2) * 0.8);
