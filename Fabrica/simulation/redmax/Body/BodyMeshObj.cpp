...
void BodyMeshObj::load_mesh(std::string filename, Vector3 scale) {
    ...
    std::string err;
    tinyobj::LoadObj(&attrib, &obj_shape, &obj_material, &err, filename.c_str(), directory_of(filename).c_str());
    ...
    for (int i = 0;i < num_shapes;i++) {
        for (int j = 0;j < (int)obj_shape[i].mesh.indices.size() / 3;j++) {
            _F.col(index) = Vector3i(obj_shape[i].mesh.indices[j * 3].vertex_index,
