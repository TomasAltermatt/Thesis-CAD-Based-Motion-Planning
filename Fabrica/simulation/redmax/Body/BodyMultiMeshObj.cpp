...
BodyMultiMeshObj::BodyMultiMeshObj(
    ...
    if (transform_type == BODY_TO_JOINT) {
        ...
        set_transform(R_ji, p_ji);
        joint->set_init_transform(math::SE(R, p));
    } else {
...
void BodyMultiMeshObj::load_mesh(std::vector<std::string> filenames, std::vector<Matrix3> Rs, std::vector<Vector3> ps, Vector3 scale) {
    ...
    for (int fi = 0; fi < filenames.size(); ++fi) {
        ...
        std::string err;
        tinyobj::LoadObj(&attrib, &obj_shape, &obj_material, &err, filenames[fi].c_str(), directory_of(filenames[fi]).c_str());
        ...
        int num_shapes = (int)obj_shape.size();
        int new_num_elements = 0;
        for (int i = 0;i < num_shapes;i++) {
            new_num_elements += (int)obj_shape[i].mesh.indices.size() / 3;
        }
        ...
        int index = 0;
        for (int i = 0;i < num_shapes;i++) {
            for (int j = 0;j < (int)obj_shape[i].mesh.indices.size() / 3;j++) {
                _F.col(index + curr_num_elements) = Vector3i(obj_shape[i].mesh.indices[j * 3].vertex_index + curr_num_vertices,
                    obj_shape[i].mesh.indices[j * 3 + 1].vertex_index + curr_num_vertices,
                    obj_shape[i].mesh.indices[j * 3 + 2].vertex_index + curr_num_vertices);
                Fi.col(index) = Vector3i(obj_shape[i].mesh.indices[j * 3].vertex_index,
                    obj_shape[i].mesh.indices[j * 3 + 1].vertex_index,
                    obj_shape[i].mesh.indices[j * 3 + 2].vertex_index);
                index++;
            }
        }
        _Fs.push_back(Fi);
