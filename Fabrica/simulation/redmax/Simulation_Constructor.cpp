...
Joint* Simulation::parse_from_xml_file(pugi::xml_node root, pugi::xml_node node,
    ...
    if ((std::string)(node.name()) == "redmax") {
            ...
            if ((std::string)(actuator.name()) == "actuator") {
                for (auto child : actuator.children()) {
                    if ((std::string)(child.name()) == "motor") {
                        ...
                        if (type == "force") {
                            int joint_ndof = it->second->_ndof;
                            VectorX ctrl_min = VectorX::Constant(joint_ndof, INT_MIN), ctrl_max = VectorX::Constant(joint_ndof, INT_MAX);
                            if (child.attribute("ctrl_range")) {
                                VectorX ctrl_range_parsed = str_to_eigen(child.attribute("ctrl_range").value());
                                int ndof_range_parsed = ctrl_range_parsed.size();
                                if (ndof_range_parsed == 2) {
                                    ctrl_min = VectorX::Constant(joint_ndof, ctrl_range_parsed(0));
                                    ctrl_max = VectorX::Constant(joint_ndof, ctrl_range_parsed(1));
                                } else if (ndof_range_parsed == 2 * joint_ndof) {
                                    ctrl_min = ctrl_range_parsed.head(joint_ndof);
                                    ctrl_max = ctrl_range_parsed.tail(joint_ndof);
                                } else {
                                    throw_error("ctrl range dof error: " + std::to_string(ndof_range_parsed));
                                }
                            }
                            ...
                            int joint_ndof = it->second->_ndof;
                            VectorX ctrl_P = VectorX::Zero(joint_ndof), ctrl_D = VectorX::Zero(joint_ndof), ctrl_I = VectorX::Zero(joint_ndof);
                            VectorX ctrl_P_parsed = str_to_eigen(child.attribute("P").value()), ctrl_D_parsed = str_to_eigen(child.attribute("D").value()), ctrl_I_parsed = VectorX::Zero(joint_ndof);
                            if (child.attribute("I")) ctrl_I_parsed = str_to_eigen(child.attribute("I").value());
                            int ndof_P_parsed = ctrl_P_parsed.size(), ndof_D_parsed = ctrl_D_parsed.size(), ndof_I_parsed = ctrl_I_parsed.size();
                            if (ndof_P_parsed == 1){
                                for (int i = 0; i < joint_ndof; ++i) ctrl_P(i) = (dtype)child.attribute("P").as_float();
                            } else if (ndof_P_parsed == joint_ndof) {
                                ctrl_P = ctrl_P_parsed;
                            } else {
                                throw_error("P dof error: " + std::to_string(ndof_P_parsed));
                            }
                            if (ndof_D_parsed == 1){
                                for (int i = 0; i < joint_ndof; ++i) ctrl_D(i) = (dtype)child.attribute("D").as_float();
                            } else if (ndof_D_parsed == joint_ndof) {
                                ctrl_D = ctrl_D_parsed;
                            } else {
                                throw_error("D dof error: " + std::to_string(ndof_D_parsed));
                            }
                            if (ndof_I_parsed == 1){
                                for (int i = 0; i < joint_ndof; ++i) ctrl_I(i) = (dtype)child.attribute("I").as_float();
                            } else if (ndof_I_parsed == joint_ndof) {
                                ctrl_I = ctrl_I_parsed;
                            } else {
                                throw_error("I dof error: " + std::to_string(ndof_I_parsed));
                            }
                            VectorX ctrl_min = VectorX::Constant(joint_ndof, INT_MIN), ctrl_max = VectorX::Constant(joint_ndof, INT_MAX);
                            if (child.attribute("ctrl_range")) {
                                VectorX ctrl_range_parsed = str_to_eigen(child.attribute("ctrl_range").value());
                            ...
                            Actuator* actuator = new ActuatorMotor(it->first + "-motor-position", it->second, ActuatorMotor::ControlMode::POS,
                                                                    ctrl_min, ctrl_max, ctrl_P, ctrl_D, ctrl_I, this->_options->_h);
                            robot->add_actuator(actuator);
                        }
                    }
                }
        ...
        for (auto contact : node.children()) 
            if ((std::string)(contact.name()) == "contact") {
                for (auto child : contact.children()) {
                    ...
                    dtype stiffness = 0.;
                    if (child.attribute("stiffness")) {
                        stiffness = (dtype)(child.attribute("stiffness").as_float());
                    } else if (root.child("default").child(child.name()).attribute("stiffness")) {
                        stiffness = (dtype)(root.child("default").child(child.name()).attribute("stiffness").as_float());
                    }
                    if ((std::string)(child.name()) == "ground_contact") {
                        ...
                        robot->add_force(force);
                    } else if ((std::string)(child.name()) == "point_to_point") {
                        auto it0 = _body_map.find(child.attribute("body0").value());
                        if (it0 == _body_map.end()) {
                            std::string error_msg = "Point to point body0 name error: " + (std::string)(child.attribute("body0").value());
                            throw_error(error_msg);
                        }
                        auto it1 = _body_map.find(child.attribute("body1").value());
                        if (it1 == _body_map.end()) {
                            std::string error_msg = "Point to point body1 name error: " + (std::string)(child.attribute("body1").value());
                            throw_error(error_msg);
                        }
                        std::string delim = ";";
                        std::vector<Vector3> pos0, pos1;
                        std::string pos0_str = child.attribute("pos0").value();
                        std::string pos1_str = child.attribute("pos1").value();
                        auto start = 0U;
                        auto end = pos0_str.find(delim);
                        while (end != std::string::npos)
                        {
                            Vector3 pos0_i = str_to_eigen(pos0_str.substr(start, end - start));
                            start = end + delim.length();
                            end = pos0_str.find(delim, start);
                            pos0.push_back(pos0_i);
                        }
                        start = 0U;
                        end = pos1_str.find(delim);
                        while (end != std::string::npos)
                        {
                            Vector3 pos1_i = str_to_eigen(pos1_str.substr(start, end - start));
                            start = end + delim.length();
                            end = pos1_str.find(delim, start);
                            pos1.push_back(pos1_i);
                        }
                        if (pos0.size() != pos1.size()) {
                            std::string error_msg = "Point to point position size error: " + std::to_string(pos0.size()) + " " + std::to_string(pos1.size());
                            throw_error(error_msg);
                        }
                        std::string frame = "joint";
                        if (child.attribute("frame")) {
                            frame = (std::string)(child.attribute("frame").value());
                        } else if (root.child("default").child(child.name()).attribute("frame")) {
                            frame = (std::string)(root.child("default").child(child.name()).attribute("frame").value());
                        }
                        ForcePointToPoint* force = new ForcePointToPoint(this, stiffness, damping, frame);
                        force->addBodies(it0->second, it1->second);
                        for (int i = 0; i < pos0.size(); i++) {
                            force->addPoints(pos0[i], pos1[i]);
                        }
                        robot->add_force(force);
                    }
                }
        ...
        if (node.child("body")) {
            ...
            // parse gravity
            if (body_node.attribute("gravity")) {
                body->_enable_gravity = body_node.attribute("gravity").as_bool();
            }
