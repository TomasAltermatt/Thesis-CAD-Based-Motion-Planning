...
def get_contact_sim_string(assembly_dir, parts=None, save_sdf=False, mat_dict=None, adaptive_sample=False):
    '''
...
<robot>
    <link name="part{part_id}">
        ...
        <body name="part{part_id}" type="SDF" filename="{assembly_dir}/{part_id}.obj" {sdf_args} adaptive_sample="{'true' if adaptive_sample else 'false'}" pos="0 0 0" quat="1 0 0 0" transform_type="OBJ_TO_JOINT" density="1" dx="0.05" res="20" mu="0" rgba="{_arr_to_str(_get_color(part_id))}"/>
    </link>
</robot>
'''
    string += f'''
...
def get_path_sim_string(assembly_dir, parts_fix, part_move, parts_removed=[], save_sdf=False, pose=None, mat_dict=None, col_th=0.01, arm_string=None, adaptive_sample=False):
    '''
...
<robot>
    <link name="part{part_id}">
        ...
        <body name="part{part_id}" type="SDF" filename="{assembly_dir}/{part_id}.obj" {sdf_args} adaptive_sample="{'true' if adaptive_sample else 'false'}" pos="0 0 0" quat="1 0 0 0" transform_type="OBJ_TO_JOINT" density="1" dx="0.05" res="20" col_th="{col_th_i}" mu="0" rgba="{_arr_to_str(color)}"/>
    </link>
</robot>
'''
...
<robot>
    <link name="part{part_id}">
        <joint name="part{part_id}" type="{joint_type}" axis="0. 0. 0." pos="{_arr_to_str(pos)}" quat="{_arr_to_str(quat)}" frame="WORLD" damping="0"/>
        <body name="part{part_id}" type="SDF" filename="{assembly_dir}/{part_id}.obj" {sdf_args} pos="0 0 0" quat="1 0 0 0" transform_type="OBJ_TO_JOINT" density="1" dx="0.05" res="20" col_th="{col_th}" mu="0" rgba="{_arr_to_str(color)}"/>
    </link>
</robot>
'''
    string += f'''
